# SPEC 03 — Asistente Ergo en el Home del colegio
...
- **El chat se monta en `pages/HomePage.tsx`** dentro de una `SeccionHome` propia, **entre**
  `StatisticsGlobal` y la sección «Requiere atención».
...
- [ ] El Home muestra una sección «Asistente Ergo» entre los contadores y «Requiere atención».
...
- **No:** modal detrás de un botón. Habría conservado `ModalBarContext`, pero repite el error del
  botón actual: esconde la única función conversacional de la pantalla.
...
| El chat en línea alarga demasiado el Home | Alto fijo de 380 px en móvil y 440 px en escritorio, con scroll interno. El `scrollIntoView` usa `block: 'nearest'` para no arrastrar la página entera. |
