...
/**
 * Reconocimiento de voz para el chat «Asistente Ergo» (Spec 04).
 *
 * Clon de `src/presentation/hooks/use-speech-recognitionGPT.tsx`. Se duplica en vez de
 * importarse por la misma razón que el servicio: la Spec 04 prohíbe depender de
 * `src/presentation/`.
 *
 * `SpeechRecognition` es una API de navegador sin tipos en `lib.dom.d.ts` estándar. El
 * asistente global la declara en `Window` con `any`; aquí se tipa localmente para no
 * ampliar la interfaz global desde el módulo de la ficha — dos declaraciones del mismo
 * `Window` en el mismo proyecto tienen que ser compatibles, y esa es una dependencia
 * invisible que se rompe sola.
 */
...
export const useReconocimientoVoz = () => {
    ...
    useEffect(() => {
        ...
        reconocedor.onresult = (evento) => {
            ...
            // `resultIndex` marca desde dónde hay novedades: reprocesar desde 0 duplicaría
            // todo lo ya acumulado en cada evento.
        ...
        // Al desmontar el tab hay que cortar el micrófono: sin esto seguiría escuchando
        // con el componente fuera de pantalla.
        return () => {
    ...
    // Los tres van con `useCallback` para que el consumidor pueda ponerlos como
    // dependencia de un efecto sin que este se re-dispare en cada render. En el tab del
    // asistente eso importa: el efecto de montaje dispara una petición.
