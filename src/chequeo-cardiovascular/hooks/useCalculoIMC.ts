/**
 * Lógica clínica clonada **sin corregir** de `src/Chequeo/hooks/useCalculoIMC.ts`.
 *
 * Cambiar una fórmula o un umbral aquí es una decisión médica, no una refactorización, así que
 * queda explícitamente fuera de esta spec. Lo que sí se hace es documentar las tres trampas.
 */

/**
 * IMC = peso / estatura².
 *
 * ⚠️ **Exige la estatura en metros.** Si el dato viene en centímetros el resultado es absurdo
 * y nada lo detecta: no hay rango de validación sobre el valor devuelto.
 */
...
/**
 * ⚠️ **No son tablas OMS/CDC.** Es una aproximación lineal propia del proyecto, acotada a
 * [0.1, 99.9]. No la presentes al usuario como un percentil clínico validado.
 */
...
/**
 * Recomendaciones por rango de IMC (adulto) o percentil (pediátrico).
 *
 * 🔴 **Bug conocido, clonado a propósito:** en adultos la rama de peso normal (`IMC < 25`)
 * devuelve exactamente el mismo texto que la de bajo peso (`IMC < 18.5`), o sea que le dice a
 * un adulto sano que necesita ganar peso. Corregirlo cambia el texto que ve un paciente y
 * requiere criterio médico: va en su propia spec, no aquí.
 */
export const UseIMCRecomendaciones = async (edad_paciente: number, IMC: number, sexo_paciente: string) => {
    ...
    if (edad_paciente >= 18) {
        ...
        if (IMC < 18.5) {
            ...
            // 🔴 Idéntico al bloque anterior. Es el bug documentado arriba, no un descuido de copia.
            recomendaciones.push(
                'Consulta con un nutricionista para ganar peso de forma saludable',
                'Aumenta el consumo de alimentos ricos en nutrientes y calorías saludables',
                'Realiza ejercicios de fuerza para ganar masa muscular',
