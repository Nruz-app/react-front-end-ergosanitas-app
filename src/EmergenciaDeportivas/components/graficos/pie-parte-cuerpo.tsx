...
export const PieChartParteCuerpo = () => {

  const { user } = useContext(LoginContext);
  const { user_email } = user;
      
  const [parteCuerpo, setParteCuerpo] = useState<LigaData>(initialValues);

  const fetchSpEstadisticaLiga = useCallback(async (): Promise<void> => {
    try {
      const { SpEstadisticaParteCuerpo } = await UseIncidentesService();
      const response = await SpEstadisticaParteCuerpo(user_email);
      setParteCuerpo(response);
    } catch (error) {
  ...
  const data: ChartData<'pie', number[], string> = {
    labels: parteCuerpo.labels,
    datasets: [
      {
        label: 'Partes del Cuerpo Más Afectadas',
        data: parteCuerpo.data,
        backgroundColor: [
              "#FFCC80", 
              "#81C784",
              "#f39c12", 
