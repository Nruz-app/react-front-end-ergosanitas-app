...
export const BarLesionesFecha = () => {

    const { user }  = useContext( LoginContext );
    const { user_email }  = user;

    const [lesiones,setLesiones] = useState<LigaData>(initialValues);


    const fetchSpEstadisticaCategoria = useCallback(async (): Promise<void> => {
        
            const {  SpEstadisticaLesionesFechas } = await UseIncidentesService() ;
            const response = await SpEstadisticaLesionesFechas(user_email);
            setLesiones(response);
    }, []);
    ...
    const data = {
        labels: lesiones.labels, 
        datasets: [
          {
            label: "Evolución de Lesiones por Jornada",
            data: lesiones.data,
            backgroundColor: [
              "#FFCC80", 
              "#81C784",
    ...
    return (
        ...
        >
            ...
            title={
                ...
                >
                Evolución de Lesiones por Jornada
                </Typography>
            }
            />
            <CardContent>
