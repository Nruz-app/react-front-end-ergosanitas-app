...
import { routesCol } from './routesCOL';
import { Loading } from './Loading';

/**
 * Navegador raíz del perfil `Colegios`.
 *
 * Es una copia de `NavigationMe` apuntando a `routesCOL`, que es el patrón que ya usan
 * `Medicos`, `Paciente` y `Emergencia Deportiva`. Como esos, **compara el perfil inline y no
 * entiende el comodín `'All'`**: solo `NavigationErgo` lo soporta. Por eso las entradas de
 * `routesCol` llevan el perfil literal.
 */
export const NavigationCol = () => {
                    ...
                    <Toolbar disableGutters>
                        ...
                        <Box 
                            ...
                            >
                            {
                                ...
                                routesCol.filter(({ status }) => status) 
                                .map(({ to, name, perfil }) => (
                                    <MenuItem key={ name } >
                                        {
                                            (user_perfil === perfil) ? (
            ...
            <Routes>
             {
                ...
                <Route path='/*' element={ <Navigate to ={ routesCol[0].to }  replace /> } />   
            </Routes>            
